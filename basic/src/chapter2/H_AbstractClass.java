...
// 반드시 추상 클래스를 상속받은 하위 클래스로 인스턴스를 생성해야 함
// abstract class 추상클래스명 (...)

// 추상 메서드 : 선언부만 존재하고 구현부는 존재하지 않는 메서드
// 반드시 추상 클래스 내부에서만 선언할 수 있음
// 반드시 추상 클래스를 상속 받은 하위 클래스에서 재정의(오버라이딩)하여 구현해야 함
// abstract 반환타입 추상메서드명(매개변수, ....);

// abstract : 추상클래스 혹은 추상메서드를 정의하는 제어자
...
class TourGuide extends Occupation {
	...
	@Override
	void work() {
		System.out.println(country + "에서 관광안내를 합니다.");
	}
